...
export function sortearNumeroNormal(geradorAleatorio) {
  const primeiro = Math.max(geradorAleatorio(), Number.EPSILON);
  const segundo = geradorAleatorio();
  return Math.sqrt(-2 * Math.log(primeiro)) * Math.cos(2 * Math.PI * segundo);
}
